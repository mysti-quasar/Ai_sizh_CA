...
from ..parser.xml_parser import parse_ledgers


async def run_ledger_sync(service: Any, sync_type: str = "incremental") -> dict[str, Any]:
    started_at = datetime.utcnow().isoformat()

    last_sync = service.cache.get_last_sync("ledgers")
    static_vars = {}
    if sync_type == "incremental" and last_sync:
        static_vars["SVEXPORTFORMAT"] = "$$SysName:XML"

    request_xml = build_export_report_request(
        report_name="List of Ledgers",
    ...
    records = parse_ledgers(tally_result.content)
    changed_records = service.cache.filter_new_or_modified("ledgers", records)

    if changed_records:
        post_result = await service.cloud_client.post_json(
            "/api/connector/sync/ledgers/",
    ...
    service.cache.set_last_sync("ledgers", datetime.utcnow().isoformat())
    service.cache.add_sync_log(
        entity="ledgers",
        sync_type=sync_type,
        status="success",
        message=f"Synced {len(changed_records)} changed ledgers",
